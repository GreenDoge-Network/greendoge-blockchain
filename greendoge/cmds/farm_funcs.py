...
from greendoge.util.network import is_localhost
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total greendoge farmed: {amounts['farmed_amount'] / units['greendoge']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['greendoge']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['greendoge']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'greendoge start wallet' and 'greendoge wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'greendoge wallet show'")
    else:
...
async def uploadfarmerdata(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    plots = await get_harvesters(harvester_rpc_port)
    blockchain_state = await get_blockchain_state(rpc_port)
    farmer_running = await is_farmer_running(farmer_rpc_port)
