...
from greendoge.cmds.configure import configure_cmd
from greendoge.cmds.farm import farm_cmd
from greendoge.cmds.init import init_cmd
from greendoge.cmds.keys import keys_cmd
from greendoge.cmds.netspace import netspace_cmd
from greendoge.cmds.passphrase import passphrase_cmd
from greendoge.cmds.plots import plots_cmd
from greendoge.cmds.show import show_cmd
from greendoge.cmds.start import start_cmd
from greendoge.cmds.stop import stop_cmd
from greendoge.cmds.wallet import wallet_cmd
...
@click.group(
    help=f"\n  Manage greendoge blockchain infrastructure ({__version__})\n",
    epilog="Try 'greendoge start node', 'greendoge netspace -d 192', or 'greendoge show -s'",
...
if not supports_keyring_passphrase():
    from greendoge.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show greendoge version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs greendoge daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by greendoge start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from greendoge.daemon.server import async_run_daemon
