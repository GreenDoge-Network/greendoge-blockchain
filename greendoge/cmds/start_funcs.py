...
from greendoge.cmds.passphrase_funcs import get_current_passphrase
from greendoge.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from greendoge.util.keychain import KeyringMaxUnlockAttempts
from greendoge.util.service_groups import services_for_groups
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    greendoge = sys.argv[0]
    process = subprocess.Popen(f"{greendoge} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)
    return process
...
async def async_start(root_path: Path, group: str, restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the greendoge daemon")
        return None
