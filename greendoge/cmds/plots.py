...
def show_plots(root_path: Path):
    from greendoge.plotting.plot_tools import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'greendoge plots add -d [dir]' and remove with"
        + " 'greendoge plots remove -d [dir]'"
        + " Scan and check plots with 'greendoge plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'greendoge init'")
...
def check_cmd(
    ...
    from greendoge.plotting.check_plots import check_plots
...
def add_cmd(ctx: click.Context, final_dir: str):
    from greendoge.plotting.plot_tools import add_plot_directory
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from greendoge.plotting.plot_tools import remove_plot_directory
