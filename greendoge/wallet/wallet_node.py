...
from greendoge.daemon.keychain_proxy import (
    KeychainProxy,
...
from greendoge.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from greendoge.protocols import wallet_protocol
from greendoge.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from greendoge.server.node_discovery import WalletPeers
...
from greendoge.server.ws_connection import WSGreenDogeConnection
...
from greendoge.util.errors import Err, ValidationError
from greendoge.util.ints import uint32, uint128
from greendoge.util.keychain import Keychain
from greendoge.util.lru_cache import LRUCache
from greendoge.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from greendoge.util.path import mkdir, path_from_root
from greendoge.wallet.block_record import HeaderBlockRecord
from greendoge.wallet.derivation_record import DerivationRecord
from greendoge.wallet.settings.settings_objects import BackupInitialized
from greendoge.wallet.transaction_record import TransactionRecord
from greendoge.wallet.util.backup_utils import open_backup_file
from greendoge.wallet.util.wallet_types import WalletType
from greendoge.wallet.wallet_action import WalletAction
from greendoge.wallet.wallet_blockchain import ReceiveBlockResult
from greendoge.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[GreenDogeServer]
    log: logging.Logger
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'greendoge keys' program.")
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSGreenDogeConnection):
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSGreenDogeConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSGreenDogeConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSGreenDogeConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSGreenDogeConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
