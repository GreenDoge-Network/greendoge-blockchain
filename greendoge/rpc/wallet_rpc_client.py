...
from greendoge.pools.pool_wallet_info import PoolWalletInfo
from greendoge.rpc.rpc_client import RpcClient
...
class WalletRpcClient(RpcClient):
    """
    Client to GreenDoge RPC, connects to a local wallet. Uses HTTP/JSON, and converts back from
    JSON into native python objects before returning. All api calls use POST requests.
    Note that this is not the same as the peer protocol, or wallet protocol (which run GreenDoge's
    ...
    async def log_in(self, fingerprint: int) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {"host": "https://backup.greendoge.dog", "fingerprint": fingerprint, "type": "start"},
            )
    ...
    async def log_in_and_restore(self, fingerprint: int, file_path) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {
                    "host": "https://backup.greendoge.dog",
    ...
    async def log_in_and_skip(self, fingerprint: int) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {"host": "https://backup.greendoge.dog", "fingerprint": fingerprint, "type": "skip"},
            )
        except ValueError as e:
            return e.args[0]
