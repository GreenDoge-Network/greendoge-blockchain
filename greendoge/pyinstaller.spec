...
ROOT = pathlib.Path(importlib.import_module("greendoge").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `greendoge` file name (which is the executable)
    and the `greendoge` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `greendoge` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("greendoge/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("greendoge-blockchain"))[0]
...
entry_points = ["greendoge.cmds.greendoge"] + [f"greendoge.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "greendoge.util.bip39"])

if THIS_IS_WINDOWS:
    greendoge_mod = importlib.import_module("greendoge")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/greendoge/util/english.txt", "greendoge/util"))
datas.append((f"{ROOT}/greendoge/util/initial-config.yaml", "greendoge/util"))
datas.append((f"{ROOT}/greendoge/wallet/puzzles/*.hex", "greendoge/wallet/puzzles"))
datas.append((f"{ROOT}/greendoge/ssl/*", "greendoge/ssl"))
...
add_binary("greendoge", f"{ROOT}/greendoge/cmds/greendoge.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/greendoge/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/greendoge/server/start_{server}.py", COLLECT_ARGS)
