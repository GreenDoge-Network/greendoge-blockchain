...
from greendoge.cmds.init_funcs import check_keys, greendoge_init
from greendoge.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from greendoge.daemon.keychain_server import KeychainServer, keychain_commands
from greendoge.daemon.windows_signal import kill
from greendoge.server.server import ssl_context_for_root, ssl_context_for_server
from greendoge.ssl.create_ssl import get_mozilla_ca_crt
from greendoge.util.greendoge_logging import initialize_logging
from greendoge.util.config import load_config
from greendoge.util.json_util import dict_to_json_str
from greendoge.util.keychain import (
    Keychain,
...
from greendoge.util.service_groups import validate_service
from greendoge.util.setproctitle import setproctitle
from greendoge.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting GreenDoge.")
    quit()
...
service_plotter = "greendoge plots create"
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("greendoge daemon exiting in 5 seconds")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    if service_command == "greendoge_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in greendoge_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    greendoge_init(root_path, should_check_keys=(not wait_for_unlock))
