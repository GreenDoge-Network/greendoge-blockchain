...
from greendoge.util.keyring_wrapper import KeyringWrapper
...
DEFAULT_USER = f"user-greendoge-{CURRENT_KEY_VERSION}"  # e.g. user-greendoge-1.8
DEFAULT_SERVICE = f"greendoge-{DEFAULT_USER}"  # e.g. greendoge-user-greendoge-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting GREENDOGE_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("GREENDOGE_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from greendoge.cmds.passphrase_funcs import prompt_for_passphrase
