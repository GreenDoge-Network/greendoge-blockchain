...
# Gets the version from the result of "greendoge version"
...
def update_version():
    with open(f"{os.path.dirname(__file__)}/greendoge-blockchain-gui/package.json") as f:
        data = json.load(f)
    ...
    with open(f"{os.path.dirname(__file__)}/greendoge-blockchain-gui/package.json", "w") as w:
