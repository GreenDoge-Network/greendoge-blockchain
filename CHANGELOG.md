...
## 1.1.2 GreenDoge Blockchain 2021-04-24
...
- There is now remote UI support. [Documents](https://github.com/GreenDoge-Network/greendoge-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/GreenDoge-Network/greendoge-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `greendoge plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/GreenDoge-Network/greendoge-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/GreenDoge-Network/greendoge-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- sha256tree has been removed from GreenDogelisp.
...
- There was a regression in GreenDoge Proof of Space ([chiapos](https://github.com/GreenDoge-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `greendoge init` then sync on first start will happen automatically. "\$GREENDOGE_ROOT" users will need to delete `$GREENDOGE_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `greendoge netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during greendoge init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `greendoge show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `greendoge show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/GreenDoge-Network/greendoge-blockchain/wiki) and placed in [INSTALL](https://github.com/GreenDoge-Network/greendoge-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/GreenDoge-Network/greendoge-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of greendoge action commands. You can run `greendoge version` or `greendoge start node` for example. Just running `greendoge` will show you more options. However `greendoge-create-plots` continues to use the hyphenated form. Also it's now `greendoge generate keys` as another example.
- GreenDoge start commands like `greendoge start farmer` and `greendoge stop node` now keep track of process IDs in a run/ directory in your configuration directory. `greendoge stop` is unlikely to work on Windows native for now. If `greendoge start -r node` doesn't work you can force the run/ directory to be reset with `greendoge start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/GreenDoge-Network/greendoge-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `greendoge start wallet-gui` has been chaned to `greendoge start wallet` which launches but the GUI and server on MacOS and Linux. `greendoge start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/GreenDoge-Network/greendoge-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `greendoge stop all` command sometimes fails, use `greendoge-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `greendoge init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `greendoge-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/GreenDoge-Network/greendoge-blockchain/wiki/Updating-beta-software). If you've set `$GREENDOGE_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `greendoge show`.

### Changed

- `greendoge-restart-harvester` has been renamed from `greendoge-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.greendoge/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the GreenDoge blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/GreenDoge-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the greendoge management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `greendoge-start-farmer &`. The [README.md](https://github.com/GreenDoge-Network/greendoge-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.greendoge/release/config, plots/ db/, wallet/ etc. This can be overridden by `export GREENDOGE_ROOT=~/.greendoge` for example which would then put the plots directory in `HomeDirectory/.greendoge/plots`.
- The libraries greendoge-pos, greendoge-fast-vdf, and greendoge-bip-158 have been moved to their own repositories: [chiapos](https://github.com/GreenDoge-Network/chiapos), [chiavdf](https://github.com/GreenDoge-Network/chiavdf), and [chaibip158](https://github.com/GreenDoge-Network/chiabip158). They are brought in by greendoge-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/GreenDoge-Network/bls-signatures).
...
- `greendoge-regenerate-keys` has been renamed `greendoge-generate-keys`.
...
- GreenDoge fullnode, farmer and harvester now default to logging to greendoge.log in the greendoge-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- Added color logging
- Now greendoge_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to greendoge_full node, greendoge_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new greendoge services were started.
...
[1.0beta4]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/GreenDoge-Network/greendoge-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/GreenDoge-Network/greendoge-blockchain/releases/tag/Alpha-1.0
