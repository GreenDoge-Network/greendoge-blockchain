...
import { mojo_to_greendoge_string, greendoge_to_mojo } from '../../../util/greendoge';
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const hegdogeck = /[\da-f]+$/gi;

    if (!hegdogeck.test(ip_val) || ip_val.value === '') {
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  ...
                  {mojo_to_greendoge_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_greendoge_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
