...
from greendoge.plotting.util import stream_plot_info_ph, stream_plot_info_pk, PlotRefreshResult, PlotRefreshEvents
from greendoge.plotting.manager import PlotManager
from greendoge.protocols import farmer_protocol
from greendoge.rpc.farmer_rpc_api import FarmerRpcApi
from greendoge.rpc.farmer_rpc_client import FarmerRpcClient
from greendoge.rpc.harvester_rpc_api import HarvesterRpcApi
from greendoge.rpc.harvester_rpc_client import HarvesterRpcClient
from greendoge.rpc.rpc_server import start_rpc_server
...
from greendoge.wallet.derive_keys import master_sk_to_wallet_sk, master_sk_to_pooling_authentication_sk
...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "gdog"), encode_puzzle_hash(new_ph_2, "gdog"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "gdog"))
            ...
            assert config["farmer"]["gdog_target_address"] == encode_puzzle_hash(new_ph, "gdog")
            assert config["pool"]["gdog_target_address"] == encode_puzzle_hash(new_ph_3, "gdog")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "gdog")
